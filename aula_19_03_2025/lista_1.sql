...
WHERE 
	idade < 40 
    AND especialidade != 'traumatologia';
...
FROM 
	consultas 
WHERE 
	data NOT BETWEEN '2024-06-14' AND '2024-12-20';

-- 5) Buscar o nome e a idade (em meses) dos pacientes
SELECT 
	nome, 
    idade*12 as idade_em_meses 
FROM 
	pacientes;
...
FROM 
	funcionarios 
WHERE 
	SUBSTRING_INDEX(nome, ' ', 1) LIKE '%a'; -- Apenas primeiro nome termina em 'A'
